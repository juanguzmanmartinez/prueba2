...
export class CapacityClientService {
    ...
    private readonly CALENDAR_UPDATE_RANGE = EndpointsParameter.CALENDAR_RANGE_UPDATE;
    ...
    public patchCalendarUpdateClient$(request: ICalendarUpdateRequestParams) {
        return this.genericService.genericPatch<IBlocked[]>(this.CALENDAR_UPDATE, request)
            .pipe(map(response => {
    ...
    public patchCalendarRangeUpdateClient$(request: ICalendarUpdateRequestParams) {
        return this.genericService.genericPatch<ICapacity[]>(this.CALENDAR_UPDATE_RANGE, request)
            .pipe(map(response => {
                const current = isArray(response) ? response : [];
