...
export class OperationsStoresEditionHomeComponent implements OnInit, OnDestroy {
    ...
    editionHomeEditStore() {
        this._router.navigate([CONCAT_PATH.opStoresStoreEdition(this.store.store)]);
    }

    editionHomeEditService(serviceType: EDeliveryServiceType) {
        const storeServiceTypePath = `${CONCAT_PATH.opStoresStoreId(this.store.store)}/${CDeliveryServiceTypeRoute[serviceType]}`;
