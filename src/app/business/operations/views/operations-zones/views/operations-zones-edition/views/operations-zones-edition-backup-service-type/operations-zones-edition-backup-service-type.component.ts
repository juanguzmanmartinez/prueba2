...
export class OperationsZonesEditionBackupServiceTypeComponent
  implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    this.serviceType = Object.keys(CDeliveryServiceTypeRoute).find((key) =>
      path.includes(CDeliveryServiceTypeRoute[key])
  ...
  private setZoneBackupServiceType(): void {
    const zoneServiceType = this.zoneBackupDetail?.serviceTypeList.find(
      (serviceType: ZoneServiceType) => serviceType.code === this.serviceType
    );
    const forceService =
      this.serviceType === EDeliveryServiceType.amPm
        ? this.zoneDetail?.zoneBackup?.forceServiceAMPM
        : this.zoneDetail?.zoneBackup?.forceServiceSCHEDULED;
    this.zoneBackupServiceType = zoneServiceType
      ? new ZoneBackupServiceType(zoneServiceType, forceService)
      : null;
    this.editionBackupServiceTypeLoader = !this.zoneBackupServiceType;
  }

  putBackupServiceType(zoneBackupUpdate: IZoneBackupUpdate): void {
    this._operationsZonesImplement
      .putZoneBackup(this.zoneDetail.id, zoneBackupUpdate)
      .subscribe(
        () => {
          this._operationsZonesEditionStore.updateZoneDetail = true;
          this._alert.alertLightSuccess(
            OperationMessages.successEditionBackupServiceType(
        ...
        () => {
          this._alert.alertError(
            OperationMessages.errorOperationEdition(this.zoneDetail.name)
          );
          this.backRoute();
        }
      );
