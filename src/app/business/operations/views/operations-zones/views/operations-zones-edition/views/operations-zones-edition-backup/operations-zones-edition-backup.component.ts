...
export class OperationsZonesEditionBackupComponent
  implements OnInit, OnDestroy
{
      ...
      .subscribe(
        () => {
          ...
          this._alert.alertLightSuccess(
            OperationMessages.successEditionBackup(this.zoneDetail.name)
          );
          this.backRoute();
        },
        () => {
  ...
  settingData(): void {
    ...
    const subscription = forkJoin([zoneTypeList$, zoneList$]).subscribe(
  ...
  setZoneList(): void {
    if (this.zoneListStored && this.zoneDetail) {
      this.zoneList = this.zoneListStored.filter(
        (zone: Zone) => zone.code !== this.zoneDetail.code
      );
    }
  }
