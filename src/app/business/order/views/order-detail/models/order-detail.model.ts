...
export class OrderDetailModel {
  ...
  constructor(data: OrderDetailResponse) {
    ...
    this.tagCompany = data.orderInfoConsolidated?.orderInfo?.companyCode
      ? data.orderInfoConsolidated.orderInfo.companyCode
      : null;
    this.iconTagCompany = data.orderInfoConsolidated?.orderInfo?.companyCode
      ? data.orderInfoConsolidated?.orderInfo?.companyCode.toLowerCase() ===
        ETypeTagBrand.inkafarma
        ? ETypeTagBrand.inkafarma
        : ETypeTagBrand.mifarma
      : null;
    ...
    this.tagOrderType = data.orderInfoConsolidated?.orderInfo?.orderType
      ...
      : '-';
    this.tagServiceType = data.orderInfoConsolidated?.orderInfo
      ?.serviceTypeShortCode
    ...
    this.promiseDate = data.orderInfoConsolidated?.orderInfo?.scheduledTime
      ? this.formatPromiseDate(
          data.orderInfoConsolidated.orderInfo.scheduledTime
        )
      : '-';

    this.timeline = data.orderStatusDetail?.statusTimeLine
      ? reorderTimeline(data.orderStatusDetail?.statusTimeLine).map(
          (timeline) => {
    ...
    this.clientInformation = data.orderInfoConsolidated?.orderInfoClient
      ? new ClientInformationModel(data.orderInfoConsolidated.orderInfoClient)
      : null;
    this.orderInformation = data.orderInfoConsolidated?.orderInfoAdditional
      ? new OrderInformationModel(
          data.orderInfoConsolidated.orderInfoAdditional,
          data?.orderInfoConsolidated?.orderInfo?.source,
          data?.orderInfoConsolidated?.orderInfo?.pickup
        )
      : null;
    this.paymentInformation = data.orderInfoConsolidated?.paymentMethodDto
      ? new PaymentInformationModel(data.orderInfoConsolidated.paymentMethodDto)
      : null;
    this.carrierInformation = data.orderInfoMotorized
      ? new CarrierInformationModel(data.orderInfoMotorized)
      : null;
