  ...
  implements ControlValueAccessor, OnInit, AfterViewInit, OnDestroy
{
  ...
  optionSelectedTemplate: TemplateRef<any>;
  @ContentChild('optionTemplate') optionTemplate: TemplateRef<any>;
  @ViewChild('inputSelectSearch', { static: true })
  inputSelectSearch: ElementRef<HTMLInputElement>;
  ...
  public filterPredicate(data: T[], filter: string): T[] {
    ...
    return data.filter((option) => {
      ...
      if (isObject(option)) {
        const concatValue = Object.keys(option)
          .map((key) => normalizeValue(option[key]))
          .join('');
        const everyValue = Object.keys(option).some((key) =>
          normalizeValue(option[key]).includes(filterValue)
        );
  ...
  validValue(value: T) {
    ...
    if (isObject(this._value)) {
      savedValue = Object.keys(this._value)
        .map((key) => normalizeValue(this._value[key]))
        .join('');
    }
    if (isObject(value)) {
  ...
  setDefaultValue() {
    if (this._value && this.selectSearch.options && this.optionList.length) {
      const index = this.optionList.findIndex((option) => {
        ...
        if (isObject(option)) {
          const concatOption = Object.keys(option)
            .map((key) => normalizeValue(option[key]))
            .join('');
          const concatValue = Object.keys(this._value)
            .map((key) => normalizeValue(this._value[key]))
            .join('');
      ...
      if (index !== -1) {
        ...
        if (
          this.selectSearch.options.length === 0 ||
          this.selectSearch.options.length !== this.optionList.length
        ) {
          timer(0, 500)
