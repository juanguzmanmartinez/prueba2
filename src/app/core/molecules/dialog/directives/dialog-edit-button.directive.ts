...
export class DialogEditButtonDirective implements AfterViewInit {
  @Input() innerClassButtonTag: string;
  constructor(
  ...
  setDialogCloseElement() {
    ...
    this.renderer.addClass(
      this.elementRef.nativeElement,
      this.innerClassButtonTag
    );
  }

  addButtonActionIconElement() {
    const buttonActionIconComponentFactory =
      this.componentFactoryResolver.resolveComponentFactory(
        ButtonActionIconComponent
      );

    const buttonActionIconComponentRef =
      buttonActionIconComponentFactory.create(this.injector);
    buttonActionIconComponentRef.instance.iconName = 'edit';
    buttonActionIconComponentRef.instance.innerClass = 'cursor-pointer';
    buttonActionIconComponentRef.instance.inlineStyle = {
      width: '32px',
      height: '32px',
    };

    this.applicationRef.attachView(buttonActionIconComponentRef.hostView);
    const buttonActionIconElement = (
      buttonActionIconComponentRef.hostView as EmbeddedViewRef<any>
    ).rootNodes[0] as HTMLElement;
    this.renderer.appendChild(
      this.elementRef.nativeElement,
      buttonActionIconElement
    );
  }
}
